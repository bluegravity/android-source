...
public class FavoritePastries {
	...
	private HashMap<Pastry, Integer> ratingsHash = new HashMap<>();
	...
	public void addPastry(Pastry pastry, int rating) {
		...
		if (ratingsHash.containsKey(pastry)) {
			// replace existing
			ratingsHash.replace(pastry, rating);
		} else {
			// add new
			ratingsHash.put(pastry, rating);
		}
	...
	public int getRatingForPastry(Pastry pastry) {
		...
		Integer i = ratingsHash.get(pastry);
		if (i != null) {
			returnValue = i;
		}
		return returnValue;
	...
	public Collection<Pastry> getPastriesForRating(int rating) {
		 ...
		 ************************************************/
		// get set of keys
		Set<Pastry> keys = ratingsHash.keySet();
		// empty Set for collecting matches
		Set<Pastry> matchingKeys = new HashSet<> ();
		
		// iterate
		for (Pastry pastry : keys) {

			if (ratingsHash.get(pastry) == rating) {
				matchingKeys.add(pastry);
			}
		}
		// return what's left - matching
		return matchingKeys;
	}
